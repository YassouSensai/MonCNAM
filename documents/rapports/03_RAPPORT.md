...
Le diagramme suivant illustre l'architecture logique de notre projet : la plateforme "MonCNAM" qui permet la gestion administrative et pédagogique de la filière FIP du CNAM. Il met en évidence la séparation entre le référentiel statique, c'est à dire les UEs, les matières et les données dynamiques comme les cours, les notes et les émargements pour le suivis des présences.

![Diagramme de classe](../images/rapport2/Diag_classe.png)

**Spécification des principales entités**

- **Utilisateurs :** Entité centrale qui va gérer l'authentifications, les droits et l'accés aux différentes fonctionnalités selon le du rôle de l'utilisateur.
- **Promotion :** Regroupe les élèves par année scolaire et par code (ex : DICASI-2026). C'est la promotion qui est concerné par un  emplois du temps donné.
- **UniteEnseignement & Matiere :** Les deux entités pour les gestion des enseignements. D'un côté le regroupement thématique des matières par semestre et crédits ECTS et de l'autre les matières.
...
### Diagramme d'état
Le diagramme d'état ci-dessous modélise le cycle de vie complet d'une séance de cours, de sa planification initiale à sa validation final par l'enseignant. Cette modélisation définit les fenêtres temporelles durant lesquelles les interactions seront réalisés tout en grarantissant l'intégrité des données d'émargement.
...
- **L'émargement :** L'étudiant scanne le code. L'API vérifie immédiatement la validité du jeton, s'assure que l'étudiant n'a pas déjà émargé (unicité) et valide la présence en base de données (qui l'étudiant appartienne bien au cours pour lequel il emmerge). L'intégration de coordonnées GPS optionnelles permet de contrer la fraude (scan à distance), mais on ne sais pas encore si le temps nous permettra d'implémenter cette fonctionnalité.
...
MonCNAM repose sur une architecture moderne typle client-serveur
